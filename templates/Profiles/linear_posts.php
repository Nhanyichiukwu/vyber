...
if (isset($posts) && count($posts)) {
    echo '<div class=" flex-nowrap flex-row foa3ulpk n1ft4jmn ofx-auto row">';
    foreach ($posts as $post): ?>
    <?php $postLink = $post->author->getUsername() . '/posts/' . $post->refid; ?>
    <div class="col-11 col-md-5 col-sm-7 col-xl-5 djmlyve7">
        <div class="cw_linear-post_post-card mmhtpn7c ajqwjj6r e8wksrsz bg-white shadow-sm">
            <div class="cw_linear-post_post-content post-content d-flex flex-column _oFb7Hd">
                <?php if ($post->hasValue('post_text')): ?>
                    <div class="post-text text-muted _qRwCre _uXY7 _3og">
                        <?php
                        $fontSize = ' fsz-16';
                        $excerptLimit = 240;
                                ...
                                <div class="copied-post">
                                    <?php if ($post->isCopiedAs('quote')): ?>
                                        <div class="<?= $fontSize ?> text-muted">
                                            <blockquote class="quote text-quoted ttdt9h2">
