...
//                    uf.appendTo('.composer-content-wrapper');
    ...
    $(document).on('click', '.submitBtn', function(e) {
        let $this = $(this),
            ...
            fm = $('#xhrs-blank-form').serializeArray(),
            csfr = $('#xhrs-blank-form [name="_csrfToken"]').val(),
            formBoundary = $('#xhrs-blank-form [name="form_boundary"]').val(),
            requestHandler = thisComposer.data('uri'),
            afterSubmitStatus = 'sent',
...
//
        ...
        if (typeof recipients !== 'undefined') {
            ...
            linkifiedRecipients = $.map(linkifiedRecipients, function (item) {
                ...
                return '<span class="_JF4usO"><a href="/' + username + '" class="_w5w _zeN4uW" role="link">' + item + '</a></span>';
            });
...
$(document).on('click', '[vibely-id="v4fU0H"]', function (e) {
    e.preventDefault();
    let theElem = $(this),
        out = $(theElem.data('target'));
    if (window.location.hostname !== 'localhost' && !navigator.onLine) {
        if (out.find('.alert').length < 1) {
            out.prepend('<div class="alert alert-danger">Oops! It seems you\'er offline</div>');
        }
        return false;
    }
    ...
    doAjax(url, function (data, status) {
        ...
        if (status === 'success') {
            ...
            result = '<div class="text-center">' + result + '</div>';
        }
...
$(document).on('page:content:arrival', function (e) {
    if (e.detail.responseData.indexOf('_Hc0qB9')) {
        let ajaxifiables = e.detail.output.find('._Hc0qB9');
        ...
        if (status === 'success' && result.status === 'success') {
            ...
            setTimeout(function () {
                let url = new URL(document.documentURI),
                    next = url.searchParams.get('redirect').ltrim('/') ?? 'feeds';
