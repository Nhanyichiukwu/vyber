...
$postLink = Router::url('/' . $post->author->getUsername() . '/posts/' . $post->refid, true);
$randID = RandomString::generateString(16, 'mixedalpha');

$author = $post->get('author');
$commentators = $reactors = $similarPosts = $series = $followers = [];

$comments = $post->get('comments');
// Everyone who has commented on this post
if (!empty($comments)) {
    $commentators = (new Cake\Collection\Collection($comments))->map(
        function ($comment) {
...
// Everyone who has reacted to this post
if (!empty($reactions)) {
    $reactors = (new Cake\Collection\Collection($reactions))->map(
        function ($reaction) {
...
if (count($commentators) || count($reactors)) {
    $followers = array_merge($commentators, $reactors);
    $followers = array_unique($followers);
}

$hashtags = (array) $post->get('hashtags');
if (count($hashtags)) {
    $hashtags = (new Cake\Collection\Collection($hashtags))->each(
        function ($hashtag) use($hashtags) {
            $usages = $this->cell('Posts::findWordUsages', [$hashtag])->render();
            $hashtags[$hashtag] = $usages;
            return $hashtags[$hashtag];
        }
    );
}
$similarPosts = $this->cell('Posts::findSimilar', [$post->get('post_text')]);
$series = $this->cell('Posts::getSeries', [$post->get('refid')]);
?>


<div class="row gutters-sm flex-lg-nowrap">
    <div class="col-8 w_IsfXJh">
        <div class="_Gy9JKV">
            <div id="post_<?= $randID ?>" class="i__post _aQtRd7eh news-item p-0 list-group-item list-group-item-action mb-0" data-url="<?= $postLink ?>">
                <div class="_1yT1Ry _SDyPWa p-4 _w5w" aria-haspopup="false" data-focusable="false"  data-id="<?= $post->refid ?>" data-uri="<?= $postLink ?>">
                    <div class="_VDKqxd">
                                        ...
                                        <div class="author-info post-meta pr-5">
                                            ...
                                            <div class="content-meta small d-flex">
                                                <div class="meta-publish-date" role="datetime">
                                                    ...
                                                    <span class="fsz-xs"><?= ucfirst
                                                        ((new App\Utility\DateTimeFormatter)->humanizeDatetime($post->get
('date_published'), $user_timezone)); ?></span>
                                                </div>
                                    ...
                                    <div class="copied-post">
                                        ...
                                        <div class="<?= $fontSize ?> text-muted">
                                            <blockquote class="quote text-quoted">
                                                    <?= Text::truncate(h($post->getBody()), 160, ['foo' => 'bar']); ?>
                                        ...
                                        <div class="text-plain <?= $fontSize ?>"><?= Text::truncate(h($post->getBody()), 240, ['ellipses' => '<a href="javascript:void(fullText())">See More</a>']); ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <?php else: ?>
                                    <div class="<?= $fontSize ?>">
                                                <?= Text::truncate(h(strip_tags($post->getBody())), 240); ?>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                <?php endif; ?>
                                    ...
                                    <div class="ml-auto _viG">
                                        <div class="_EowDPO _viG bgc-grey-900">
                                            ...
                                            <?php endif; ?>
                                                ...
                                                <?php if ($post->has('comments') && !$post->isEmpty('comments')): ?>
...
//                        The user on whose wall this post is being shared from
                        ...
                        <div id="<?= $randID; ?>CommentContainer" class="comments-container collapsable collapse comment-area _Ad53 _lyAO5Z _ezNu bg-white mb-n3 mx-n3 p-4 pb-4 mt-3">
                            ...
                            <?php endif; ?>
                                <div id="<?= $randID ?>CommentComposer" class="composer comment-composer form-container _viG" data-uri="<?= Router::url('/' . $post->author->getUsername() . '/' . 'posts/' . $post->refid . '/comments/add_comment') ?>">
    ...
    <aside class="col-4 w_JhJQoD">
        <div class="_apdrrp">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title"><span class="icon"><i class="mdi mdi-24px mdi-book-open-page-variant"></i></span> <span class="_y9cA">Series</span></h6>
                </div>
                <div class="card-footer"></div>
            </div>
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title"><span class="icon"><i class="mdi mdi-24px mdi-pound"></i></span> <span
                            class="_y9cA">Hashtags</span></h6>
                </div>
                <div class="card-footer"></div>
            </div>
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title"><span class="icon"><i class="mdi mdi-24px mdi-post-outline"></i></span> <span class="_y9cA">Similar Posts</span></h6>
                </div>
                <div class="card-footer"></div>
            </div>
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title"><span class="icon"><i class="mdi mdi-24px mdi-account-multiple-check-outline"></i></span> <span
                            class="_y9cA">People Following this Posts</span></h6>
                    <?php if (count($followers)): ?>
                        <ul class="users-list clearfix">
                            <?php foreach ($followers as $follower): ?>
                                <li>
                                    <img src="dist/img/user1-128x128.jpg" alt="User Image">
                                    <a class="users-list-name" href="#"><?= __("{fullname}", ['fullname' =>
                                            $follower->getFullName()]) ?></a>
                                    <span class="users-status">Online</span>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif; ?>
                ...
                <div class="card-footer">
                    <?= $this->Html->link(__('More'), $postLink . '/followers'); ?>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title"><span class="icon"><i class="mdi mdi-24px mdi-comment-text-multiple-outline"></i></span> <span
                            class="_y9cA">Live Engagement</span></h6>
                </div>
                <div class="card-footer"></div>
            </div>
        </div>
