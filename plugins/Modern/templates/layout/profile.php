...
<body id="cw-app_profile-view" class="<?= $offcanvasState  . ' ' . $appView ?> <?= $appAccent ?> app-body off-canvas fixed-header h-auto"
      data-app-accent="<?= $appAccent ?>"
      data-page-layout="off-canvas"
      <?php if (isset($appUser)): ?>
      data-app-user="@<?= $appUser->getUsername() ?>"
      <?php endif; ?>
...
<aside id="off-canvas" class="col-aside d-lg-block d-none">
    <div
        ...
        <?= $this->element('Profile/sidebar/dashboard', ['appUser' => $appUser]) ?>
    </div>
</aside>
<?php endif; ?>
<div id="app-canvas" class="app-canvas d-block w-100">
    <?= $this->element('App/off_canvas_header', ['control' => null]) ?>
        ...
        <main role="main" class="main container-fluid :only-when-on-mobile">
            <div class="row">
                <div id="pageContent" class="col-auto col-lg-11 col-main
                flex-lg-grow-0 flex-md-fill n1ft4jmn ofjtagoh pb-5 pb-md-0 utzwucqe">
                    <div class="_oFb7Hd">
                        <div class="_oFb7Hd h-100">
                            <?php if ($this->isPageHeaderEnabled()): ?>
                                <header
                                    class="bg-white border-bottom my-0 n1ft4jmn page-header py-2 py-md-3 mx-n3 q3ywbqi8">
        ...
        <?php 
//                mb_strtolower(
            echo RandomString::generateString(32, 'mixed', 'alpha');
//        ); 
        ?>
                        </div>
                    </div>
    ...
    <aside class="right-docker _Y7wf _ycGkU4 border-left _kkz9 _uXY7 _wf7p ofy-auto u8aoy6la z_Gtob">
    </aside>
    <?php if (isset($appUser)): ?>
