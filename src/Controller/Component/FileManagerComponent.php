...
class FileManagerComponent extends Component
{
     ...
     */
    public static function saveFile($file, $destination, $filename_prefix = null) {
        $result = null;
        if ($file instanceof UploadedFile) {
            $result = self::_saveAsCakeUploadedFile($file, $destination, $filename_prefix);
        } else {
            $result = self::_saveAsPHPNativeUploadedFile($file, $destination, $filename_prefix);
        }

        return $result;
    }

    protected static function _saveAsCakeUploadedFile(
        UploadedFile $file,
    ...
    )
    {
        ...
        $filename_prefix = is_null($filename_prefix)
            ? time()
            : $filename_prefix;
        $newFilename = $filename_prefix. '_' . Security::randomString() . '.' . $extension;
     ...
     */
    protected static function _saveAsPHPNativeUploadedFile(
        $file,
        string $destination,
        string $filename_prefix = null
    )
     ...
     * @return Folder
     */
    public static function getUploadDir(string $user_refid, $path = null)
    {
        $uploadDir = new Folder(
            static::UPLOAD_DIR . 'user-' . $user_refid,
            true, 0755
        );
        ...
        if (is_readable($withAddedPath)) {
            ...
            $uploadDir = new Folder($withAddedPath, true, 0755);
        }
        return $uploadDir;
    }
