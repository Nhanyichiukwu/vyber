...
<div class="d-b e__sBs mb-2">
    ...
    <img class="avatar avatar-md avatar-placeholder fl-l mr-3 e__sBs-l" src="<?= h($comment->author->profile_image_url)?>">
    <?php else: ?>
    <div class="avatar avatar-md avatar-placeholder fl-l mr-3 e__sBs-l"></div>
    <?php endif; ?>
    <div class="e__sBs-r kUR235 offset-2 w-auto">
        ...
        <?php // if (isset($comment->replies) && count($comment->replies)): ?>
        <div class="comment-replies" role="replies-thread" id="e__post-comment-<?= $this->Number->format(h($comment->id)); ?>-replies">
            <?php // foreach ($comment->replies as $reply): ?>
                <?= $this->element('Singletons/comment_reply_singleton', ['comment' => $comment]); ?>
            <?php // endforeach; ?>
        </div>
        <?php // endif; ?>
    </div> 
