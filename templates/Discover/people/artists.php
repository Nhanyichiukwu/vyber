...
$this->set('page_title', 'Discover Artists');
...
<section class="mb-2 mgriukcz bg-white">
        ...
        <div class="container-fluid py-2">
            ...
            <div id="filter-pane" class="collapse <?= isset($get['filtered']) ? 'show' : '' ?> border-top mt-2">
                <div class="row gutters-sm">
                    ...
                    <div class="col-auto">
                        <label for="role" class="lh-sm mb-1 small">Role:</label>
                        <select id="role" name="artist_role"
                                class="form-select form-select-sm kfjlsihm neh5467l"
                                aria-label=".form-select-sm example">
                            <option value="">Select Role</option>
                            <?php foreach ($roles as $role): ?>
                                <option value="<?= $role->slug ?>" <?= isset($get['artist_role'])
                                && $get['artist_role'] === $role->slug ? 'selected' : ''?>><?= $role->name ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-auto">
                        <label for="music-category" class="lh-sm mb-1 small">Music Category:</label>
                        <select id="music-category" name="music_category"
                                class="form-select form-select-sm kfjlsihm neh5467l"
                                aria-label=".form-select-sm example">
                            <option value="">Select Category</option>
                            <?php foreach ($categories as $category): ?>
                                <option value="<?= $category->slug ?>" <?= isset($get['music_category'])
                                && $get['music_category'] === $category->slug ? 'selected' : ''?>><?= $category->name ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
    ...
    $dataSrces = 'ajaxify/users/artists?t=' . Security::encrypt('music') . '&token=' . $token;
    if (!empty($filters)) {
        $dataSrces .= '&' . $filters;
    }
    ?>
