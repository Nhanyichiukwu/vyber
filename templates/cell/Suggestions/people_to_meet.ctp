<?php if (count($peopleToMeet)): ?>
    ...
    <?php foreach ($peopleToMeet as $personToMeet): ?>
            ...
            <div class="offset-1 pl-3">
                <a class="mb-0"><strong><?= h($personToMeet->fullname); ?></strong> <small class="text-muted-dark">@<?= h($personToMeet->username); ?></small></a>
                <div class="about-content-block"><?= h($personToMeet->about); ?></div>
                <div class="meta-data text-small text-muted-dark">
                    <span class="personality"><?= h($personToMeet->personality); ?></span> |
                    <span class="location"><?= h($personToMeet->location); ?></span> |
                    <span class="genre"><?= h($personToMeet->genre); ?></span>
                </div>
                <div class="">
                    <button class="btn btn-outline-primary btn-sm"><span class="mdi mdi-account-plus"></span> Connect</button>
                    <button class="btn btn-danger btn-sm"><span class="mdi mdi-cancel"></span> Connect</button>
                </div>
            </div>
        </li>
        <li class="col-md-6 learfix py-3">
