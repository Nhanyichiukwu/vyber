...
<div class="page-header row gutters-lg justify-content-between align-items-center">
        <div class="col"><h2 class="page-title"><?= __('Songs') ?> <small class="page-subtitle">You have <?= count
            ($songs) ?>
                    songs</small></h2></div>
                ...
                <table id="media-list" class="table table-hover table-outline table-vcenter text-nowrap card-table click-action">
                    ...
                    <?php foreach ($songs as $song): ?>
                        <tr
                            ...
                            data-id="<?= h($song->get('refid')); ?>"
                            data-title="<?= h($song->get('title')); ?>"
                            data-uri="<?= Router::url('songs/' . h($song->get('refid')), true) ?>">
                            ...
                            <td>
                                <div><?= h($song->get('title')); ?></div>
                                <?php if (! $song->isEmpty('description')): ?>
                                <div class="small text-muted"><?= Text::truncate(h($song->get('description')), 100, ['ellipsis' => '...']); ?></div>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                                <?php
                                ...
                                $badgeState = $statusDictionary[h($song->get('status'))];
                                ?>
                                <div class="badge badge-pill badge-<?= $badgeState ?>"><?= ucfirst(h($song->get('status'))) ?></div>
                            </td>
                            <td>
                                <?= ucfirst(h($song->get('privacy'))); ?>
                            </td>
                            <td>
                                <?= ucfirst(h($song->get('language') ?? 'english')); ?>
                            </td>
                            <td>
                                <?= $this->formatTime($song->get('created')); ?>
                            </td>
                            <td class="text-center">
                                <div class="item-action dropdown">
                                    <a href="javascript:void(0)" data-toggle="dropdown" class="menu"><i class="menu-icon mdi mdi-dots-vertical"></i></a>
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="play/<?= h($song->get('refid')); ?>" class="dropdown-item"><i class="dropdown-icon mdi mdi-video"></i> Play</a>
                                        <a href="play/<?= h($song->get('refid')); ?>" class="dropdown-item"><i class="dropdown-icon mdi mdi-video"></i> Copy Link</a>
